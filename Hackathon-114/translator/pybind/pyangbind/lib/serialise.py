...
class pybindIETFXMLEncoder(object):
    ...
    def yname_ns_func(parent_namespace, element, yname):
        ...
        if element._yang_type == "identityref" and element._changed():
            ...
            # Edited by Patrick: Check if it is a List (Leaf-list). 
            # If a list, must get each element and append it to avoid error.
            # If not, just use the single element.
            if element._pybind_generated_by == "TypedListType":
                for el in element:
                    ns_map.append(
                        (el._enumeration_dict[el]["@module"], el._enumeration_dict[el]["@namespace"])
                    )
            else:            
                ns_map.append(
