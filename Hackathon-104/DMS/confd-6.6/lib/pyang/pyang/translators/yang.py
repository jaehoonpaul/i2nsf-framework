...
class YANGPlugin(plugin.PyangPlugin):
    ...
    def add_opts(self, optparser):
        optlist = [
            ...
            optparse.make_option("--yang-canonical-alpha",
                                 dest="yang_canonical_alpha",
                                 action="store_true",
                                 help="Print in canonical order ++"),
...
def emit_stmt(ctx, stmt, fd, level, prev_kwd_class, indent, indentstep):
    ...
    if len(stmt.substmts) == 0:
        ...
        elif ctx.opts.yang_canonical_alpha:
            substmts = grammar.sort_canonical(stmt.keyword, stmt.substmts, True)
        else:
