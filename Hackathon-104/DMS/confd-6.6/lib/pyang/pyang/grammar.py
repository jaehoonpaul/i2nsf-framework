...
def sort_canonical(keyword, stmts, alphabetical=False):
    ...
    for (kw, _spec) in flatten_spec(subspec):
        res.extend(_sort_alpha(kw, alphabetical,
                               [stmt for stmt in stmts if stmt.keyword == kw]))
    # then copy all other statements (extensions)
    res.extend([stmt for stmt in stmts if stmt not in res])
    return res

_allow_alphabetical_sort = (
    'augment',
    'feature',
    'grouping',
    'identity',
...
def _sort_alpha(kw, alphabetical, stmts):
    if not alphabetical:
        return stmts
    if kw not in _allow_alphabetical_sort:
        return stmts
    return sorted(stmts, key=lambda stmt: stmt.arg)
