    ...
    while (item) {
        ...
        if (strcmp(item->callpoint, "hcp") == 0) {
            ...
            case C_SET_ELEM:
                ...
                if ((dlist = find_iface(running_db,
                                        &(keypath->v[4][0]),
                                        &(keypath->v[1][0]))) != NULL) {
            ...
            case C_CREATE:
                ...
                if ((dlist = find_host(running_db, &(keypath->v[3][0])))
                    != NULL) {
                    hp = (struct host*) dlist->val;
                         ...
                         long next)
{
    ...
    if (next == -1) { /* first call */
        Dlist *hlink = find_host(running_db, &(keypath->v[2][0]));
        struct host *hp;
        if (hlink == NULL)
            return reteof(tctx);
                          ...
                          confd_hkeypath_t *keypath)
{
    ...
    Dlist *list;
    struct iface *iface;
    if ((list = find_iface(running_db, &(keypath->v[4][0]),
                           &(keypath->v[1][0]))) == NULL) {
        confd_data_reply_not_found(tctx);
        return CONFD_OK;
    }
    ...
    if (strcmp(tok,"show") == 0) {
        ...
        if ((hostlink = find_host(running_db, &v)) == NULL) {
            ...
            if (
                ((domain = strtok(NULL, sep)) != NULL) &&
                ((defgw =  strtok(NULL, sep)) != NULL)) {
                add_host(running_db, new_host(name, domain, defgw));
        ...
        if (currhost[0] == 0) {
            ...
            CONFD_SET_STR(&v,tok);
            if ((iflink = find_iface(running_db, &h, &v)) != NULL) {
                free(iflink->val);
