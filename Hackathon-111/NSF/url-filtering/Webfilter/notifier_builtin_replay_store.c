...
static void send_notif_ddos (struct confd_identityref idref_attack_type, char *src_ip, struct confd_datetime start_time, int attack_rate)
{
	confd_tag_value_t vals[8];
	int i = 0;
	idref_attack_type.id = nsfmi_tcp_con_flood;
	idref_attack_type.ns = nsfmi__ns;

	struct in_addr attack_src_ip;
	inet_pton(AF_INET, src_ip, &(attack_src_ip.s_addr));
	...
	CONFD_SET_TAG_XMLBEGIN(&vals[i], nsfmi_i2nsf_nsf_detection_ddos, nsfmi__ns); i++;
	CONFD_SET_TAG_IDENTITYREF(&vals[i], nsfmi_attack_type, idref_attack_type); i++;
	CONFD_SET_TAG_DATETIME(&vals[i], nsfmi_start_time, start_time); i++;
	CONFD_SET_TAG_IPV4(&vals[i], nsfmi_attack_src_ip, attack_src_ip); i++;
	CONFD_SET_TAG_UINT32(&vals[i], nsfmi_attack_rate, attack_rate);i++;
	CONFD_SET_TAG_IDENTITYREF(&vals[i],  nsfmi_acquisition_method,  idref_acqui);  i++;
	CONFD_SET_TAG_IDENTITYREF(&vals[i],   nsfmi_emission_type,   idref_emission);  i++;
	CONFD_SET_TAG_XMLEND(&vals[i], nsfmi_i2nsf_nsf_detection_ddos, nsfmi__ns); i++;
	send_notification(vals,i);
}
...
int main(int argc, char **argv)
{
    ...
    bool over = false;
    bool ddos_notif = false;
    while (1){
	char *sep2 = " -\n";
	FILE *f = fopen("attack_DDoS.txt","r");
	char buf2[BUFSIZ];
	while (fgets(&buf2[0], BUFSIZ,f)!=NULL) {
		char *cp2 = strtok(&buf2[0],sep2); //YEAR
		cp2 = strtok(NULL, sep2); //MONTH
       		//printf("%s\n",cp2);
	        cp2 = strtok(NULL, sep2); //DATE
        	//printf("%s\n",cp2);
	        cp2 = strtok(NULL, sep2); //TIME
        	//printf("%s\n",cp2);
        	cp2 = strtok(NULL, sep2); //ATTACKER IP
	        char *attacker_ip = cp2;

		struct confd_identityref idref_attack_type;
		idref_attack_type.id = nsfmi_tcp_con_flood;
		idref_attack_type.ns = nsfmi__ns;

		struct confd_datetime start_time;
		getdatetime(&start_time);

		if (!ddos_notif){
			printf("SENDING DDOS NOTIFICATION\n");
		        send_notif_ddos(idref_attack_type, attacker_ip, start_time, 1000); 
			ddos_notif = true;
		}
	}
       fclose(f);
    ...
    while (fgets(&buf[0],BUFSIZ,p)!=NULL)
    {
      ...
      if (strcmp(cp, "total")!=0){
        ...
        if ((percent > 80)&&(!over)) {
          over = true;
          send_notif_monitoring(nsfmi_MEM_USAGE_ALARM,percent,80,nsfmi_subscription,nsfmi_on_change,"url_filtering",nsfmi_high,"Memory Usage Exceeded The Threshold");
  	  ...
  	  printf("SENDING ALARM DATA\n");
   	  ...
   	  system("python log.py");
          ...
          send_notif_resources(json_object_get_string(status),json_object_get_int(cpu_usage),json_object_get_int(memory_usage),json_object_get_int(disk_usage),json_object_get_int(disk_left), json_object_get_int(in_traffic_speed),json_object_get_int(out_traffic_speed),"url_filtering");
