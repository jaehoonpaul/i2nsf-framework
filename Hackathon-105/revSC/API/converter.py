...
class DataConverter:
  ...
  def initializeDB(self):
    nsfdb = MySQLdb.connect(host="localhost", user="root", db="nsfdb")
    nsfcur = nsfdb.cursor()
    ...
    #nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('webservers', 54, '221.159.112.91')")
    #nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('webservers', 55, '221.159.112.97')")
  ...
  def convertData(self):
    # register the endpoint information
    nsfdb = MySQLdb.connect(host="localhost", user="root", db="nsfdb")
    nsfcur = nsfdb.cursor()

    if self.extractedlist[51]:
      if self.extractedlist[55]:
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[51][0]+"', 43, '"+self.extractedlist[55][0]+"')")
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[51][0]+"', 50, '"+self.extractedlist[55][0]+"')")
      if self.extractedlist[56]:
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[51][0]+"', 44, '"+self.extractedlist[56][0]+"')")
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[51][0]+"', 51, '"+self.extractedlist[56][0]+"')")
      print("Regi 1\n")
      client_socket2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
      client_socket2.connect(('127.0.0.1', 55590))
      client_socket2.send("IP Registration Success\n")


    if self.extractedlist[57]:
      nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[57][0]+"', 84, '80,443')")
      nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[57][0]+"', 124, 'http_and_https_flood')")
      if self.extractedlist[61]:
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[57][0]+"', 54, '"+self.extractedlist[61][0]+"')")
      if self.extractedlist[62]:
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[57][0]+"', 55, '"+self.extractedlist[62][0]+"')")
      print("Regi 2\n")
      client_socket2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
      client_socket2.connect(('127.0.0.1', 55590))
      client_socket2.send("Device Registration Success\n")
   
    if self.extractedlist[76]:
      if self.extractedlist[78]:
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[76][0]+"', 114, '"+self.extractedlist[78][0]+"')")
        nsfcur.execute("INSERT INTO endpointtable (ename, id, data) VALUES ('"+self.extractedlist[76][0]+"', 123, 'url-filtering')")
      print("Regi 3\n")
      client_socket2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
      client_socket2.connect(('127.0.0.1', 55590))
      client_socket2.send("URL Registration Success\n")


    nsfdb.commit()
    nsfcur.close()
    nsfdb.close()
    if self.extractedlist[51] or self.extractedlist[57] or self.extractedlist[76]:  return False

    self.matchData(0, 0)   # match policy-name
    self.matchData(1, 4)   # match rule-name
    #self.matchData(15, 118) # match packet-per-second
    self.matchData(18, 120) # match ingress-action
    self.matchData(18, 121) # match egress-action

    # match start-time
    if self.extractedlist[5]:
      self.requiredlist[8].append(self.extractedlist[5][0]+':00Z')

    # match end-time
    if self.extractedlist[6]:
      self.requiredlist[9].append(self.extractedlist[6][0]+':00Z')
  ...
  def policyprovisioning(self, cfglist, requestIP, requestPORT):
    ...
    for i in range(nsflen):
      ...
      if isSelected:
        ...
        if rows[0][0] == 'False':
          ...
          selected_nsf.append(self.nsflist[i])
          selectednsfstring += (self.nsflist[i]+',')
          socketAPI.request_nsf('127.0.0.1', 55590, self.nsflist[i]) 
    ...
    """
    temp = "{ \"nsf_names\":[\""
    for z in range(0, len(selected_nsf)):
       temp += selected_nsf[z]
       if z < len(selected_nsf)-1:
           temp += "\", \""
       else:
           temp += "\"] }\n"
